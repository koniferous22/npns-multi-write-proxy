ARG IMAGE_TAG_MULTI_WRITE_PROXY_ALPINE
ARG NODE_ENV
FROM node:$IMAGE_TAG_MULTI_WRITE_PROXY_ALPINE AS base_node_alpine
RUN echo "Building multi-write-proxy with NODE_ENV=\"${NODE_ENV}\""

WORKDIR /usr/src/multi-write-proxy
COPY package.json package-lock.json ./
RUN npm ci

# VERSION 1 of the image: npm start:dev
FROM base_node_alpine AS npns_production
ENV MULTI_WRITE_PROXY_START_COMMAND=start
# Source code is copied and built in this image
COPY . /usr/src/multi-write-proxy
RUN npm run build
